...
async function registerAllTools(server, options = {}) {
  ...
  try {
    ...
    // Phase 3: Register dynamic module management tools
    await registerDynamicManagementTools(server);
...
/**
 * Phase 3: Register dynamic module management tools
 * These tools allow runtime management of the tool registry itself!
 * @param {McpServer} server - The MCP server instance
 */
async function registerDynamicManagementTools(server) {
  console.log('[MCP SDK] 🔧 Registering dynamic module management tools...');
  
  // Tool: Get module status and hot-reload information
  server.tool(
    'registry_get_status',
  ...
  // Tool: Load a module dynamically
  server.tool(
    ...
    async (args) => {
      try {
        const fullPath = require('path').resolve(__dirname, args.modulePath);
  ...
  // Tool: Reload a module (hot-reload)
  server.tool(
  ...
  // Tool: Toggle hot-reload for the entire system
  server.tool(
...
module.exports = {
  ...
  DynamicRegistryDB,
  registerDynamicManagementTools
};
