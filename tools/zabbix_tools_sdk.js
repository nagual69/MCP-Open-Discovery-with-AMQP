  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        severityFilter: {
          type: "string",
          enum: ["not_classified", "information", "warning", "average", "high", "disaster"],
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        eventType: {
          type: "string",
          enum: ["trigger", "discovery", "autoregistration", "internal"],
  ...
  switch (name) {
    ...
    case "zabbix_get_inventory":
      try {
        ...
        const search = args.hostFilter ? { host: args.hostFilter } : undefined;
        const filter = {};
        ...
        if (args.inventoryMode) {
          ...
          filter.inventory_mode = modeMap[args.inventoryMode];
        }
        ...
        return {
          content: [
            {
              type: "text",
              text: `Successfully retrieved inventory for ${formattedHosts.length} hosts from Zabbix:\n\n${JSON.stringify(formattedHosts, null, 2)}`
            }
          ]
        };
      } catch (error) {
    ...
    case "zabbix_get_problems":
      try {
        ...
        if (args.severityFilter) {
          ...
          severities = [severityMap[args.severityFilter]];
        }

        if (args.acknowledged !== undefined) {
          filter.acknowledged = args.acknowledged ? 1 : 0;
        }

        if (args.recent) {
          timeFrom = Math.floor(Date.now() / 1000) - (24 * 60 * 60); // 24 hours ago
        }
    ...
    case "zabbix_get_events":
      try {
        ...
        if (args.eventType) {
          ...
          filter.source = sourceMap[args.eventType];
        }
        ...
        if (args.timeFrom) {
          timeFromSeconds = parseTimeString(args.timeFrom);
        }
        
        if (args.timeTill) {
          timeTillSeconds = parseTimeString(args.timeTill);
        }
    ...
    case "zabbix_get_triggers":
      try {
        ...
        const params = {
          output: ['triggerid', 'description', 'expression', 'status', 'priority', 'lastchange', 'value'],
          selectHosts: ['host', 'name'],
          filter: filter,
          limit: args.limit || 100,
          sortfield: 'priority',
        ...
        if (args.templated !== undefined) {
          params.templated = args.templated;
        }
...
function parseTimeString(timeStr) {
  ...
  // Handle relative time strings like "1h", "1d", etc.
  ...
  if (match) {
    ...
    return Math.floor((now - (amount * multipliers[unit])) / 1000);
  }
