...
function getCredential(id) {
  ...
  return {
    ...
    sshKey: credential.sshKey ? decrypt(credential.sshKey) : undefined,
    oauthToken: credential.oauthToken ? decrypt(credential.oauthToken) : undefined,
    certificate: credential.certificate ? decrypt(credential.certificate) : undefined,
  };
}
...
function listCredentials(type) {
  ...
  return Object.entries(store)
    .filter(([_, credential]) => !type || credential.type === type)
    .map(([id, credential]) => ({ id, type: credential.type, username: credential.username, url: credential.url }));
}
...
function rotateKey(newKey) {
  ...
  for (const id in store) {
    const credential = store[id];
    try {
      if (credential.password) credential.password = encryptWithNewKey(decryptWithOldKey(credential.password));
      if (credential.apiKey) credential.apiKey = encryptWithNewKey(decryptWithOldKey(credential.apiKey));
      if (credential.sshKey) credential.sshKey = encryptWithNewKey(decryptWithOldKey(credential.sshKey));
      if (credential.oauthToken) credential.oauthToken = encryptWithNewKey(decryptWithOldKey(credential.oauthToken));
      if (credential.certificate) credential.certificate = encryptWithNewKey(decryptWithOldKey(credential.certificate));
    } catch (error) {
