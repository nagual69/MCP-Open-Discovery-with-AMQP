...
- Validation snapshot on 2026-03-17: `npm run typecheck` passes and `npm test` passes against the typed stdio runtime with 60/60 tool checks green, but `npm run typecheck:strict` still fails. The remaining strict-mode work is concentrated in typed credentials storage, net-utils request/annotation handling, plugin-manager exact-optional typing, AMQP transport exact-optional typing, OAuth/HTTP transport request typing, and missing ambient package declarations for `express`, `adm-zip`, and `better-sqlite3`.
- The typed deployment source of truth now lives under `src/docker/` with one multi-stage `Dockerfile` and one profile-driven `compose.yml`. That replaces the old split between `docker-compose.yml`, `docker-compose-amqp.yml`, `docker-compose-snmp-testing.yml`, `docker-compose-zabbix-testing.yml`, and `docker-file-mcpod-server.yml` for typed-runtime deployments.
- The consolidated Compose stack fixes an actual operational conflict in the legacy files: Zabbix web and Keycloak both wanted host port 8080. In the new stack, Keycloak keeps internal port 8080 but publishes on host port 8081 so OAuth and Zabbix lab profiles can run together.
