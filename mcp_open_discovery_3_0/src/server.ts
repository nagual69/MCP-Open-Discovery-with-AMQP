...
export function createAppConfig(): AppConfig {
  ...
  return {
    ...
    oauth: {
      ...
      realm: process.env.OAUTH_REALM || 'mcp-open-discovery-3',
    ...
    amqp: {
      ...
      exchange: process.env.AMQP_EXCHANGE || 'mcp-open-discovery-3.notifications',
      queuePrefix: process.env.AMQP_QUEUE_PREFIX || 'mcp-open-discovery-3.discovery',
      ...
      recoveryRetryInterval: Number.parseInt(
        process.env.AMQP_RECOVERY_RETRY_INTERVAL || process.env.AMQP_RETRY_INTERVAL || '30000',
        10,
      ),
      recoveryMaxRetries: Number.parseInt(
        process.env.AMQP_RECOVERY_MAX_RETRIES || '-1',
        10,
      ),
      recoveryBackoffMultiplier:
        process.env.AMQP_EXPONENTIAL_BACKOFF === 'false'
          ? 1
          : Number.parseFloat(process.env.AMQP_RECOVERY_BACKOFF_MULTIPLIER || '1.5'),
      recoveryMaxRetryInterval: Number.parseInt(
        process.env.AMQP_RECOVERY_MAX_RETRY_INTERVAL || process.env.AMQP_MAX_RETRY_INTERVAL || '300000',
        10,
      ),
    },
  };
...
export async function createMcpServer(): Promise<McpServer> {
  ...
  serverInitialization = (async () => {
    const server = new McpServer(
      {
        name: 'mcp-open-discovery-3',
        version: '2.0.0',
        description: 'TypeScript-first network discovery tools via Model Context Protocol',
      },
      {
        capabilities: {
