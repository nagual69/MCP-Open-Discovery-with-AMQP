...
async function resolvePluginValidationTarget(pluginNameArg) {
    const pluginName = pluginNameArg || 'net-utils';
    const manifestPath = path_1.default.join(process.cwd(), 'plugins', 'src', pluginName, 'mcp-plugin.json');
    ...
    const expectedRegistrationTotal = expectedCapabilities.tools + expectedCapabilities.resources + expectedCapabilities.prompts;
    ...
    return {
        ...
        manifest,
        pluginZip: path_1.default.join(process.cwd(), 'plugins', 'builtin', `${manifest.name}@${manifest.version}.zip`),
    };
}
async function main() {
    const pluginNameArg = process.argv[2];
    const workspaceTempBase = path_1.default.join(os_1.default.tmpdir(), 'mcpod-typed-validation');
    ...
    try {
        ...
        if (!activeRegistration ||
            ...
            throw new Error(`Expected capabilities ${JSON.stringify(validationTarget.expectedCapabilities)} for ${installed.pluginId}, got ${JSON.stringify(actualCapabilities)}`);
        }
