...
function asPromptResult(description, text) {
    return {
        description,
...
function normalizePersistCiMap(value) {
    if (value === true) {
        return 'yes';
    }
    if (value === false) {
...
function generateProxmoxClusterValidationPrompt(args) {
...
exports.promptDefinitions = [
    {
        ...
        description: 'CMDB CI Classification Standards - Provides guidance for classifying Configuration Items (CIs) in the CMDB according to ITIL v4 standards',
        inputSchema: types_1.CmdbPromptInputShape,
        handler: async (args) => generateCmdbClassificationPrompt(args),
    },
    {
        ...
        inputSchema: types_1.NetworkTopologyPromptInputShape,
        handler: async (args) => generateNetworkTopologyPrompt(args),
    },
    {
        name: 'security_assessment',
        description: 'Infrastructure Security Assessment - Provides security analysis and recommendations based on infrastructure discovery',
        inputSchema: types_1.SecurityAssessmentPromptInputShape,
        handler: async (args) => generateSecurityAssessmentPrompt(args),
    },
    {
        name: 'capacity_planning',
        description: 'Infrastructure Capacity Planning - Analyzes resource utilization and provides capacity planning recommendations',
        inputSchema: types_1.CapacityPlanningPromptInputShape,
        handler: async (args) => generateCapacityPlanningPrompt(args),
    },
    {
        name: 'incident_response',
        description: 'Incident Response Playbook - Provides incident response guidance based on infrastructure alerts and monitoring data',
        inputSchema: types_1.IncidentResponsePromptInputShape,
        handler: async (args) => generateIncidentResponsePrompt(args),
    },
    {
        name: 'proxmox_cluster_validation',
        description: 'Proxmox Cluster Validation - Asks for missing Proxmox inputs, exercises cluster discovery in JSON and markdown modes, and builds a CMDB memory map for the cluster',
        inputSchema: types_1.ProxmoxClusterValidationPromptInputShape,
        handler: async (args) => generateProxmoxClusterValidationPrompt(args),
    },
];
