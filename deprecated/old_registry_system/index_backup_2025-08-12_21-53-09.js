...
// CRITICAL FIX: Import standard zod-to-json-schema library for simple parameter tools
// This fixes the keyValidator._parse MCP SDK bug by using proper JSON Schema conversion
...
/**
 * Convert Zod schema to JSON Schema with proper library handling
 * 
 * CRITICAL FIX for keyValidator._parse MCP SDK bug:
 * - Use standard zod-to-json-schema library for simple parameter tools (58 tools)
 * - Use custom conversion only for array parameter tools (3 tools)
 * - Remove problematic properties that break MCP SDK validation
 * 
 * This fixes the schema validation issue that prevented tool execution
 */
function zodToJsonSchema(zodSchema, options = {}) {
  try {
    // Detect if this is an array parameter tool that needs custom conversion
    const hasArrays = hasArrayParameters(zodSchema);
    
    if (hasArrays) {
      console.log('🔧 Using CUSTOM conversion for array parameter tool');
      // Keep custom conversion for array tools (3 tools: snmp_get, snmp_get_next, zabbix_get_alerts)
      return customZodToJsonSchema(zodSchema, options);
    } else {
      console.log('⚡ Using STANDARD library conversion for simple parameter tool');
      // Use standard library for simple parameter tools (58 tools)
      const schema = standardZodToJsonSchema(zodSchema, {
        target: 'jsonSchema7',
        ...options
      });
      
      // CRITICAL FIX: Remove problematic properties that break MCP SDK
      console.log(`🔍 BEFORE sanitization:`, {
        hasSchema: '$schema' in schema,
        additionalProperties: schema.additionalProperties,
        hasDefs: '$defs' in schema,
        hasDefinitions: 'definitions' in schema
      });
      
      // 1. Remove $schema meta-property that confuses MCP validation
      delete schema.$schema;
      
      // 2. Change additionalProperties from false to true for MCP compatibility
      if (schema.additionalProperties === false) {
        schema.additionalProperties = true;
      }
      
      // 3. Remove any $defs or definitions that might confuse MCP
      delete schema.$defs;
    ...
    console.error('❌ Schema conversion failed, using fallback:', error);
    // Fallback to custom conversion on error
    return customZodToJsonSchema(zodSchema, options);
  }
}

/**
 * Custom Zod to JSON Schema conversion for array parameter tools
 * This is the original manual conversion function, now scoped to array tools only
 */
...
async function _registerFreshTools(server, registry, validator) {
  ...
  const toolModules = [
    ...
    { 
      ...
      category: 'Test',
      loader: () => require('../test_simple_sdk')
    },
    { 
      name: 'test_raw_schema_sdk',
      category: 'Test',
      loader: () => require('../test_raw_schema_sdk')
    },
    { 
      name: 'debug_validation_sdk',
      category: 'Debug',
      loader: () => require('../debug_validation_sdk')
    },
    { 
      ...
      if (module.tools && Array.isArray(module.tools)) {
        ...
        for (const tool of module.tools) {
          ...
          if (validationResult && validationResult.valid) {
            ...
            // Register using the appropriate MCP SDK method with parameter validation wrapper
            // CRITICAL FIX: Wrap handlers to bypass MCP SDK schema validation issues
            const safeHandler = async (args) => {
              try {
                // Bypass any schema validation issues by calling directly
                console.log(`[Registry] [DEBUG] Tool ${tool.name} called with args:`, Object.keys(args || {}));
                return await module.handleToolCall(tool.name, args || {});
              } catch (error) {
                console.error(`[Registry] ❌ Tool ${tool.name} execution failed:`, error.message);
                return {
                  content: [
                    {
                      type: "text",
            ...
            // Validated - Function ID#1007 - MCP SDK REGISTRATION (FIXED!)
            if (registrationMethod === 'server.tool') {
              ...
              server.tool(tool.name, tool.description, registrationSchema, safeHandler);
            } else {
              ...
              server.registerTool(tool.name, toolConfig, safeHandler);
            }
