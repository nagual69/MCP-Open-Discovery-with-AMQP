 ...
 * * Proxmox Testing Options:
 *   --proxmox-server=hostname   Specify Proxmox server hostname
 ...
 *   --proxmox-vmid=id           Specify Proxmox VM ID for testing
 *   
 * Groups:
...
const proxmoxConfig = {
  ...
  vmid: process.env.PROXMOX_VMID || '100',
...
async function promptForProxmoxConfig() {
  ...
  console.log('  PROXMOX_SERVER, PROXMOX_USER, PROXMOX_PASSWORD, PROXMOX_NODE, PROXMOX_VMID');
  ...
  proxmoxConfig.vmid = await promptInput('Proxmox VM ID for testing', proxmoxConfig.vmid);
  ...
  console.log(`  VM ID: ${proxmoxConfig.vmid}${process.env.PROXMOX_VMID ? ' (from environment)' : ''}`);
  ...
  try {
    ...
    if (toolName.startsWith('proxmox_')) {
      ...
      if (resultText.includes('authentication failure')) {
        ...
        // Check if the resource exists but is of the wrong type
        const isWrongResourceType = 
          (toolName === 'proxmox_get_vm_details' && isContainer(nodeId, vmId)) ||
          (toolName === 'proxmox_get_container_details' && isVM(nodeId, vmId));
  ...
  try {
    ...
    args.forEach(arg => {
      ...
      // Proxmox configuration
        ...
        proxmoxConfig.vmid = arg.substring(15);
      }
